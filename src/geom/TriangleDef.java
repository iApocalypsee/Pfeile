...
/**
 * TODO I don't know if this version is bug free at all.
 * @author Josip
 * @version 13.05.2014
 */
@Deprecated
public class TriangleDef implements Triangle {
	...
	public TriangleDef(PointRef p1, PointRef p2, PointRef p3) {
		this.p1 = p1;
		this.p2 = p2;
		this.p3 = p3;
	}
	...
	@Override
	public double area() {
		VectorDef v1 = new VectorDef(p1, p2), v2 = new VectorDef(p2, p3), v3 = new VectorDef(p3, p1), longest = null;
		PointRef head = null;
		PointRef larger = null, lesser = null;
		double res = 0;

		if(v1.compareTo(v2) == 1 && v1.compareTo(v3) == 1) {
			longest = v1;
			head = p3;
		} else if(v2.compareTo(v1) == 1 && v2.compareTo(v3) == 1) {
			longest = v2;
			head = p1;
		} else if(v3.compareTo(v1) == 1 && v3.compareTo(v2) == 1) {
			longest = v3;
			head = p2;
		}
		...
		if(longest.getStartX() > longest.getEndX()) {
			diffx = (longest.getStartX() - longest.getEndX()) * -1;
		} else {
			diffx = longest.getEndX() - longest.getStartX();
		}

		if(longest.getStartY() > longest.getEndY()) {
			diffy = (longest.getStartY() - longest.getEndY()) * -1;
		} else {
			diffy = longest.getEndY() - longest.getStartY();
		}

		PointRef intersection = longest.getStart().add(new PointRef(diffx, diffy));
		VectorDef height = new VectorDef(intersection, head);
		return height.straightLength();
	}
	...
	public Rectangle getBounds() {
		return new Rectangle((int) lessX(), (int) lessY(), (int) (lessX() + (greatX() - lessX())), (int) (lessY() + (greatY() - lessY())));
	}
