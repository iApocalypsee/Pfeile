...
public abstract class Field extends Component implements AttackContainer, GUIUpdater {
	 ...
	 * field has not been discovered by the com.github.pfeile.player yet.
	 */
	private boolean inFogOfWar = true;

	/**
	 * Indicates whether the com.github.pfeile.player can currently see this field.
	 */
	...
	/**
	 * Returns <b>only</b> the entities on the field that match specified type.
...
//		}
	 ...
	 * '<code> Mechanics.widthStreching </code> ' etwas ge�ndert. 
	 * 
	 * <code> updateFields </code> entspricht updateGUI, blo� f�r alle Felder. 
	 *  <code> updateFields </code> muss nur einmal f�r alle Felder aufgerufen werden.
	 *  <code> updateFields </code> ist als Thread mit Priorit�t 3 geschrieben.
	 *  
			...
			public void run() {
				...
				for (int x = 0; x < Mechanics.worldSizeX; x++) {
					for (int y = 0; y < Mechanics.worldSizeY; y++) {
						...
						// wenn die Karte insegsammt zu gro� ist, muss sie  verkleinert werden: 
...
//							System.out.println(world.getViewport().getShiftX() + " " + world.getViewport().getShiftY());
						...
						// wenn der Randeinschub zu klein (unter 0 --> au�erhalb des Bildschirms ist) ist, 
						// wird er wieder reingeschoben inset... = 0 und dann die Vergr��erungsfakorern angeglichen 
	...
	/**
	 * Returns <code>true</code>, if the active com.github.pfeile.player can watch currently this field.
	 * For convenience with {@link com.github.pfeile.comp.Component}, the method has to be named
	...
	/**
	 * Returns a boolean value indicating whether the field is accessable
	 * for regular players. This method check can be surpassed in some situations
